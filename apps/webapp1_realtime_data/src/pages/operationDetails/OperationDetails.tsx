...
const OperationDetailsPage = () => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-white p-3 border border-gray-300 rounded shadow">
          <p className="text-sm font-medium">{`Time: ${new Date(new Date(label).getTime() + 2 * 60 * 60 * 1000).toLocaleTimeString(
            "en-US",
            { hour12: false }
          )}`}</p>
  ...
  if (error) {
    return (
      <Box sx={{ p: 3 }}>
        <Alert severity="error">{error || "Error loading operation data"}</Alert>
      </Box>
    )
  }
  ...
  return (
    <Box sx={{ p: 3 }}>
        ...
        <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2 }}>
          ...
          >
            ...
            <Typography variant="body2" color="text.secondary">
              Born: {new Date(operation.Pacient.datum_rojstva).toLocaleDateString("en-US")}
            </Typography>
          </Box>
          ...
          <Box sx={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(300px, 1fr))", gap: 2 }}>
            {availableDevices.map((device) => (
              <Card
                ...
                variant="outlined"
                sx={{
                  borderColor: selectedDevices.has(device.deviceName) ? "primary.main" : "divider",
                  bgcolor: selectedDevices.has(device.deviceName) ? "primary.50" : "background.paper",
                  transition: "all 0.2s",
                }}
              >
                <CardContent sx={{ p: 2, "&:last-child": { pb: 2 } }}>
                  ...
                  <Box sx={{ display: "flex", alignItems: "center", gap: 1, flexWrap: "wrap" }}>
                    <Chip
                      ...
                      color={selectedDevices.has(device.deviceName) ? "primary" : "default"}
                      variant="outlined"
                    />
                    <Chip
                      label={`~ ${getAverageMeasurementsPerMetric(device)} measurements / metric`}
                      size="small"
                      color={selectedDevices.has(device.deviceName) ? "primary" : "default"}
                      variant="outlined"
                    />
      ...
      {operation.meritve && operation.meritve.length > 0 ? (
        <>
          <Typography variant="h5" gutterBottom sx={{ mt: 3, display: "flex", alignItems: "center" }}>
            ...
            {selectedDevices.size < availableDevices.length && (
              <Typography variant="body2" color="text.secondary" component="span" sx={{ ml: 2 }}>
                (Showing {selectedDevices.size} of {availableDevices.length} devices)
              </Typography>
            )}
          </Typography>
          ...
          {filteredDeviceGroups.length > 0 ? (
            filteredDeviceGroups.map((device) => (
              <Paper sx={{ p: 3, mb: 3 }} key={device.deviceName}>
                ...
                >
                  {device.displayName}
                  <Chip label={`${device.metrics.length} metrics`} size="small" variant="outlined" sx={{ ml: 2 }} />
                </Typography>
                ...
                {device.metrics.map((group, idx) => (
                  <Box key={`${group.deviceId}-${group.field}`} sx={{ mb: 4 }}>
                    ...
                    >
                      ...
                      />
                      <Chip
                        label={`Min: ${Math.min(...group.data.map((d) => d.value)).toFixed(1)} ${
                          getProperUnit(group)
                        }`}
                        size="small"
                        color="info"
                      ...
                      />
                      <Chip
                        label={`Max: ${Math.max(...group.data.map((d) => d.value)).toFixed(1)} ${
                          getProperUnit(group)
                        }`}
                        size="small"
                        color="error"
                      ...
                      <Chip
                        label={`Average: ${(
                          group.data.reduce((sum, d) => sum + d.value, 0) / group.data.length
                    ...
                    <div style={{ width: "100%", overflowX: "auto", overflowY: "hidden" }}>
                      <div style={{ display: "flex", alignItems: "center" }}>
                        ...
                        <div style={{ width: Math.max(group.data.length * 15, 800), height: 300 }}>
                          <LineChart
                          ...
                          >
                            ...
                            <XAxis
                              dataKey="time"
                              tickFormatter={(time) =>
                                new Date(new Date(time).getTime() + 2 * 60 * 60 * 1000).toLocaleTimeString(
                                  "en-US",
                                  { hour12: false }
                                )
                              }
                            ...
                            />
                            <YAxis domain={["auto", "auto"]} padding={{ bottom: 0 }} tickCount={6} allowDecimals />
            ...
            <Paper sx={{ p: 2, mb: 3 }}>
              <Alert severity="info">No devices selected for display. Please select devices above.</Alert>
            </Paper>
          )}
        </>
      ) : (
        <Paper sx={{ p: 2, mb: 3 }}>
          ...
          <Alert severity="info">No vital sign measurements available for this operation.</Alert>
        </Paper>
      )}
    </Box>
  )
