...
const OperationDetailsPage = () => {
	...
	const getProperUnit = (group: MeasurementGroup): string => {
		...
		// Ventilator
		if (
			f.includes("vol.ch0.mechanical_ventilator") ||
			f.includes("tidalvolume")
		) {
			return "mL";
		}
		if (
			f.includes("rf.ch0.mechanical_ventilator") ||
			f.includes("respiratoryrate")
		) {
			return "bpm";
		}
	...
	useEffect(() => {
		const fetchOperationDetails = async () => {
			try {
				...
				const response = await api.get<{ data: OperationData }>(
					`/operations/${id}/data`
				);
