...
class OperationService {
  ...
  generateLabel(field) {
    const fieldMappings = {
	  ...
	  "metrics_pip.ch0.mechanical_ventilator": "PIP (Peak Inspiratory Pressure)",
	  "metrics_peep.ch0.mechanical_ventilator": "PEEP (Positive End-Expiratory Pressure)",
  ...
  getUnit(field) {
	...
	console.log(`Starting transformation of ${influxResults.length} raw measurements`)
	...
	const measurementGroups = Object.values(groupedData).map((group) => {
		...
		console.log(`Created group: ${label} with ${group.data.length} data points`)
	...
	console.log(`Transformation complete: ${measurementGroups.length} groups created`)
  ...
  async getOperationWithMeasurements(id) {
    ...
    const toISOString = (dateStr, timeStr) => new Date(`${dateStr}T${timeStr}`).toISOString()

    const start = toISOString(datum, cas_zacetka)
    const endDate = new Date(`${datum}T${cas_konca}`)
    endDate.setSeconds(endDate.getSeconds() + 1)
    const stop = endDate.toISOString()

    console.log(`Fetching measurements for operation ${roomUuid} from ${start} to ${stop}`)

    const rawMeasurements = await influx.queryMeasurements(roomUuid, start, stop)
