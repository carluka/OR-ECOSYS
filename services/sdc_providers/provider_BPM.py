...
def set_provider_data(prov: provider.SdcProvider):
    loc = location.SdcLocation(fac=os.getenv('ref_fac', default='fac'),
...
def run_provider():
    ...
    try:
        ...
        metric = prov.mdib.descriptions.handle.get_one('numeric.ch0.vmd0')
        ...
        while True:
            try:
                ...
                current_value = max(min_hr, min(max_hr, current_value + delta))
                hr_decimal = decimal.Decimal(current_value).quantize(decimal.Decimal('1.0'))
                ...
                with prov.mdib.metric_state_transaction() as mgr:
                    ...
                    logger.info(f'Set metric value to {hr_decimal} {unit}')
